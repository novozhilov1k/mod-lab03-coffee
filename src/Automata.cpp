...
Automata::Automata(const std::string& menuFile)
    : state(STATES::OFF), cash(0), selectedDrink(-1) {
...
void Automata::showCookingProgress() {
  ...
  std::cout << "\n[АВТОМАТ] Приготовление напитка \""
            << menu[selectedDrink] << "\"" << std::endl;
...
void Automata::coin(int value) {
  if (state == STATES::WAIT || state == STATES::ACCEPT) {
    ...
    cash += value;
    printMessage("Внесено: " + std::to_string(value) +
                 " руб. Всего: " + std::to_string(cash) + " руб.");
...
void Automata::choice(int index) {
  ...
  selectedDrink = index;
  printMessage("Выбран напиток: " + menu[selectedDrink] +
               " (Цена: " + std::to_string(prices[selectedDrink]) + " руб.)");
...
void Automata::check() {
  ...
  if (cash >= prices[selectedDrink]) {
    ...
    printMessage("Недостаточно средств. Не хватает: " +
                 std::to_string(needed) + " руб.");
    setState(STATES::ACCEPT);
  }
}
...
std::vector<std::string> Automata::getMenu() const {
  ...
  for (size_t i = 0; i < menu.size(); ++i) {
    result.push_back(std::to_string(i + 1) + ". " + menu[i] +
